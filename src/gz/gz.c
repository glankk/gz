...
void main_hook()
{
  ...
  if (cheats_magic)
    (*(uint8_t*) z64_magic_addr) = ((*(uint8_t*) z64_magic_capacity_addr) +
                                    1) * 0x30;
  if (cheats_sticks) {
  ...
  if (frames_queued == -1 && input_ptr->pad & BUTTON_Z) {
    ...
    if (input_ptr->pad & BUTTON_D_RIGHT) {
      ...
      /* locked rotation */
      (*(uint16_t*)z64_actor_rot_addr) = stored_rot[tp_slot].y;
