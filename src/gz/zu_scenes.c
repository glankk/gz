#include "zu.h"

struct zu_scene_info zu_scene_info[] =
{
  {
    "inside the deku tree",
    2, (const char*[])
    {
      "from kokiri forest",
      "from gohma's lair",
    },
  },
  {
    "dodongo's cavern",
    2, (const char*[])
    {
      "from death mountain trail",
      "from king dodongo's lair",
    },
  },
  {
    "inside jabu jabu's belly",
    2, (const char*[])
    {
      "from zora's fountain",
      "water switch room, from barinade's lair",
    },
  },
  {
    "forest temple",
    3, (const char*[])
    {
      "from sacred forest meadow",
      "from phantom ganon's lair",
      "ceiling crush room",
    },
  },
  {
    "fire temple",
    2, (const char*[])
    {
      "from death mountain crater",
      "from volvagia's lair",
    },
  },
  {
    "water temple",
    2, (const char*[])
    {
      "from lake hylia",
      "entrance, from morpha's lair",
    },
  },
  {
    "spirit temple",
    5, (const char*[])
    {
      "from desert colossus",
      "from twinrova's lair",
      "from silver gauntlets chest",
      "from mirror shield chest",
      "(bad map, crashes)",
    },
  },
  {
    "shadow temple",
    4, (const char*[])
    {
      "from graveyard",
      "outside of bongo bongo's lair",
      "from bongo bongo's lair",
      "(crashes)",
    },
  },
  {
    "bottom of the well",
    2, (const char*[])
    {
      "from kakariko village",
      "from kakariko village (unused)",
    },
  },
  {
    "ice cavern",
    2, (const char*[])
    {
      "from zora's fountain",
      "near block puzzle room",
    },
  },
  {
    "ganon's tower",
    3, (const char*[])
    {
      "from inside ganon's castle",
      "from ganondorf's lair",
      "facing boss key door",
    },
  },
  {
    "gerudo training ground",
    1, (const char*[])
    {
      "from gerudo's fortress",
    },
  },
  {
    "thieves' hideout",
    13, (const char*[])
    {
      "from gerudo's fortress, 1st floor left",
      "from gerudo's fortress, 1st floor middle left",
      "from gerudo's fortress, 1st floor middle",
      "from gerudo's fortress, 2nd floor middle right",
      "from gerudo's fortress, 1st floor right",
      "from gerudo's fortress, 2nd floor middle left",
      "from gerudo's fortress, 2nd floor right",
      "from gerudo's fortress, 3rd floor right",
      "from gerudo's fortress, 3rd floor middle right",
      "from gerudo's fortress, 4th floor middle",
      "from gerudo's fortress, 2nd floor left",
      "from gerudo's fortress, above prison",
      "from gerudo's fortress, 3rd floor left",
    },
  },
  {
    "inside ganon's castle",
    8, (const char*[])
    {
      "from ganon's castle",
      "from ganon's tower",
      "after completing forest trial",
      "after completing water trial",
      "after completing shadow trial",
      "after completing fire trial",
      "after completing light trial",
      "after completing spirit trial",
    },
  },
  {
    "ganon's tower collapse",
    8, (const char*[])
    {
      "ganondorf's lair, higher",
      "ganondorf's lair, lower",
      "burning rocks, higher",
      "burning rocks, lower",
      "stalfos fight, higher",
      "stalfos fight, lower",
      "staircase, higher",
      "staircase, exit",
    },
  },
  {
    "inside ganon's castle collapse",
    1, (const char*[])
    {
      "from ganon's tower collapse",
    },
  },
  {
    "treasure box shop",
    1, (const char*[])
    {
      "from market",
    },
  },
  {
    "gohma's lair",
    1, (const char*[])
    {
      "from inside the deku tree",
    },
  },
  {
    "king dodongo's lair",
    1, (const char*[])
    {
      "from dodongo's cavern",
    },
  },
  {
    "barinade's lair",
    1, (const char*[])
    {
      "from inside jabu jabu's belly",
    },
  },
  {
    "phantom ganon's lair",
    1, (const char*[])
    {
      "from forest temple",
    },
  },
  {
    "volvagia's lair",
    1, (const char*[])
    {
      "from fire temple",
    },
  },
  {
    "morpha's lair",
    1, (const char*[])
    {
      "from water temple",
    },
  },
  {
    "twinrova's lair",
    3, (const char*[])
    {
      "from spirit temple",
      "unused",
      "battle arena, from nabooru knuckle",
    },
  },
  {
    "bongo bongo's lair",
    1, (const char*[])
    {
      "from shadow temple",
    },
  },
  {
    "ganondorf's lair",
    1, (const char*[])
    {
      "from ganon's tower",
    },
  },
  {
    "ganon's tower exterior",
    8, (const char*[])
    {
      "from ganondorf fight",
      "from ganondorf's lair, higher",
      "from ganondorf's lair, lower",
      "from burning rocks, higher",
      "from burning rocks, lower",
      "from stalfos fight, higher",
      "from stalfos fight, lower",
      "from staircase",
    },
  },
  {
    "market entrance, day",
    3, (const char*[])
    {
      "from market",
      "from hyrule field",
      "from lots 'o pots",
    },
  },
  {
    "market entrance, night",
    3, (const char*[])
    {
      "from market",
      "from hyrule field",
      "from lots 'o pots",
    },
  },
  {
    "market entrance, ruins",
    3, (const char*[])
    {
      "from market",
      "from hyrule field",
      "from lots 'o pots",
    },
  },
  {
    "back alley, day",
    5, (const char*[])
    {
      "from market, right",
      "from market, left",
      "from bombchu shop",
      "from back alley guy's house",
      "from puppy woman's house",
    },
  },
  {
    "back alley, night",
    5, (const char*[])
    {
      "from market, right",
      "from market, left",
      "from bombchu shop",
      "from back alley guy's house",
      "from puppy woman's house",
    },
  },
  {
    "market, day",
    11, (const char*[])
    {
      "from market entrance",
      "from hyrule castle",
      "from temple of time exterior",
      "from back alley, right",
      "from back alley, left",
      "from potion shop",
      "from bazaar",
      "from bombchu bowling alley",
      "from shooting gallery",
      "from happy mask shop",
      "from treasure box shop",
    },
  },
  {
    "market, night",
    11, (const char*[])
    {
      "from market entrance",
      "from hyrule castle",
      "from temple of time exterior",
      "from back alley, right",
      "from back alley, left",
      "from potion shop",
      "from bazaar",
      "from bombchu bowling alley",
      "from shooting gallery",
      "from happy mask shop",
      "from treasure box shop",
    },
  },
  {
    "market, ruins",
    11, (const char*[])
    {
      "from market entrance",
      "from ganon's castle",
      "from temple of time exterior",
      "from back alley, right",
      "from back alley, left",
      "from potion shop",
      "from bazaar",
      "from bombchu bowling alley",
      "from shooting gallery",
      "from happy mask shop",
      "from treasure box shop",
    },
  },
  {
    "temple of time exterior, day",
    2, (const char*[])
    {
      "from market",
      "from temple of time",
    },
  },
  {
    "temple of time exterior, night",
    2, (const char*[])
    {
      "from market",
      "from temple of time",
    },
  },
  {
    "temple of time exterior, ruins",
    2, (const char*[])
    {
      "from market",
      "from temple of time",
    },
  },
  {
    "know-it-all brothers' house",
    1, (const char*[])
    {
      "from kokiri forest",
    },
  },
  {
    "house of twins",
    1, (const char*[])
    {
      "from kokiri forest",
    },
  },
  {
    "mido's house",
    1, (const char*[])
    {
      "from kokiri forest",
    },
  },
  {
    "saria's house",
    1, (const char*[])
    {
      "from kokiri forest",
    },
  },
  {
    "carpenter boss's house",
    1, (const char*[])
    {
      "from kakariko village",
    },
  },
  {
    "back alley guy's house",
    1, (const char*[])
    {
      "from back alley",
    },
  },
  {
    "bazaar",
    2, (const char*[])
    {
      "from kakariko village",
      "from market",
    },
  },
  {
    "kokiri shop",
    1, (const char*[])
    {
      "from kokiri forest",
    },
  },
  {
    "goron shop",
    1, (const char*[])
    {
      "from goron city",
    },
  },
  {
    "zora shop",
    1, (const char*[])
    {
      "from zora's domain",
    },
  },
  {
    "kakariko potion shop",
    3, (const char*[])
    {
      "from kakariko village, front",
      "from kakariko village, front (unused)",
      "from kakariko village, back"
    },
  },
  {
    "market potion shop",
    1, (const char*[])
    {
      "from market",
    },
  },
  {
    "bombchu shop",
    2, (const char*[])
    {
      "empty",
      "from back alley"
    },
  },
  {
    "happy mask shop",
    1, (const char*[])
    {
      "from market",
    },
  },
  {
    "link's house",
    2, (const char*[])
    {
      "from bed",
      "from kokiri forest"
    },
  },
  {
    "puppy woman's house",
    1, (const char*[])
    {
      "from back alley",
    },
  },
  {
    "stable",
    1, (const char*[])
    {
      "from lon lon ranch",
    },
  },
  {
    "impa's house",
    2, (const char*[])
    {
      "from kakariko village, door",
      "from kakariko village, roof"
    },
  },
  {
    "lakeside laboratory",
    2, (const char*[])
    {
      "from lake hylia",
      "eyedrops timeout",
    },
  },
  {
    "carpenter's tent",
    1, (const char*[])
    {
      "from gerudo valley",
    },
  },
  {
    "gravekeeper's hut",
    1, (const char*[])
    {
      "from graveyard",
    },
  },
  {
    "great fairy's fountain, upgrades",
    4, (const char*[])
    {
      "from death mountain trail",
      "from death mountain crater",
      "from hyrule castle",
      "from kakariko village, potion shop front",
    },
  },
  {
    "fairy's fountain",
    1, (const char*[])
    {
      "grotto",
    },
  },

  {
    "great fairy's fountain, spells",
    3, (const char*[])
    {
      "from zora's fountain",
      "from hyrule castle",
      "from desert colossus",
    },
  },
  {
    "grottos",
    14, (const char*[])
    {
      "default",
      "big skulltula",
      "business scrub",
      "two redeads",
      "three business scrubs",
      "webbed",
      "octoroc",
      "two business scrubs, nut upgrade",
      "two wolfos",
      "bombable wall",
      "two business scrubs, green potion"
      "tektite",
      "forest stage",
      "cow",
    },
  },
  {
    "grave with redead",
    1, (const char*[])
    {
      "from graveyard",
    },
  },
  {
    "grave with shield",
    1, (const char*[])
    {
      "from graveyard",
    },
  },
  {
    "royal family's tomb",
    2, (const char*[])
    {
      "from graveyard",
      "after learning sun's song",
    },
  },
  {
    "shooting gallery",
    2, (const char*[])
    {
      "from kakariko village",
      "from market",
    },
  },
  {
    "temple of time",
    8, (const char*[])
    {
      "from temple of time exterior",
      "unused",
      "after pulling or placing master sword",
      "facing away from spiritual stones spot",
      "after first time pulling master sword",
      "after recieving light arrow",
      "after recieving light arrow (unused)",
      "from prelude warp",
    },
  },
  {
    "chamber of sages",
    2, (const char*[])
    {
      "default",
      "forest medallion cutscene",
    },
  },
  {
    "castle courtyard, guards day",
    2, (const char*[])
    {
      "from hyrule castle",
      "from castle courtyard, zelda",
    },
  },
  {
    "castle courtyard, guards night",
    2, (const char*[])
    {
      "from hyrule castle",
      "from castle courtyard, zelda",
    },
  },
  {
    "cutscene map",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "dampes grave & windmill",
    3, (const char*[])
    {
      "from graveyard",
      "from kakariko village",
      "after learning song of storms",
    },
  },
  {
    "fishing pond",
    1, (const char*[])
    {
      "from lake hylia",
    },
  },
  {
    "castle courtyard, zelda",
    2, (const char*[])
    {
      "from castle courtyard, guards",
      "after triforce cutscene",
    },
  },
  {
    "bombchu bowling alley",
    1, (const char*[])
    {
      "from market",
    },
  },
  {
    "lon lon ranch buildings",
    3, (const char*[])
    {
      "talon's house, from lon lon ranch",
      "back tower, from lon lon ranch",
      "talon's house, after cucco game",
    },
  },
  {
    "lots 'o pots",
    1, (const char*[])
    {
      "from market entrance",
    },
  },
  {
    "granny's potion shop",
    1, (const char*[])
    {
      "from kakariko village",
    },
  },
  {
    "ganon battle arena",
    1, (const char*[])
    {
      "from inside ganon's castle collapse",
    },
  },
  {
    "house of skulltula",
    1, (const char*[])
    {
      "from kakariko village",
    },
  },
  {
    "hyrule field",
    18, (const char*[])
    {
      "after zelda's escape, from impa escort",
      "from kakariko village",
      "from zora's river, land",
      "from lost woods",
      "from lake hylia",
      "from gerudo valley",
      "from lon lon ranch",
      "from market entrance",
      "near lake hylia",
      "owl escort from lake hylia",
      "near north-west river",
      "from lon lon ranch, south fence jump",
      "from lon lon ranch, west fence jump",
      "from lon lon ranch, east fence jump",
      "from zora's river, water",
      "from lon lon ranch, gate jump",
      "after learning song of time",
      "after impa escort cutscene",
    },
  },
  {
    "kakariko village",
    16, (const char*[])
    {
      "from hyrule field",
      "from death mountain trail",
      "from graveyard",
      "from bazaar",
      "from bottom of the well",
      "from impa's house, door",
      "from carpenter boss's house",
      "from granny's potion shop",
      "from windmill",
      "from potion shop, front",
      "from shooting gallery",
      "from house of skulltula",
      "from potion shop, back",
      "after learning nocturne",
      "owl escort from death mountain trail",
      "from impa's house, roof",
    },
  },
  {
    "graveyard",
    9, (const char*[])
    {
      "from kakariko village",
      "from shadow temple",
      "from gravekeeper's hut",
      "from dampe's grave",
      "from grave with shield",
      "from grave with redead",
      "from royal family's tomb",
      "from nocturne warp",
      "from shadow temple warp",
    },
  },
  {
    "zora's river",
    5, (const char*[])
    {
      "from hyrule field, land",
      "top of waterfall",
      "from zora's domain",
      "from hyrule field, water",
      "from lost woods",
    },
  },
  {
    "kokiri forest",
    13, (const char*[])
    {
      "forest medallion cutscene",
      "from inside the deku tree",
      "from lost woods, bridge",
      "from link's house",
      "from kokiri shop",
      "from know-it-all brothers' house",
      "from lost woods",
      "top of links house",
      "from house of twins",
      "from mido's house",
      "from saria's house",
      "after receiving the kokiri's emerald",
      "talking to the deku tree sprout",
    },
  },
  {
    "sacred forest meadow",
    4, (const char*[])
    {
      "from lost woods",
      "from forest temple",
      "from minuet warp",
      "from forest temple warp",
    },
  },
  {
    "lake hylia",
    10, (const char*[])
    {
      "from hyrule field",
      "from gerudo valley",
      "from water temple",
      "near hyrule field entrance",
      "from lakeside laboratory",
      "near serenade entrance",
      "from fishing pond",
      "from zora's domain",
      "from serenade warp",
      "from water temple warp",
    },
  },
  {
    "zora's domain",
    5, (const char*[])
    {
      "from zora's river",
      "from zora's fountain",
      "from zora shop",
      "eyeball frog timeout",
      "from lake hylia",
    },
  },
  {
    "zora's fountain",
    6, (const char*[])
    {
      "after receiving the zora's sapphire",
      "from inside jabu jabu's belly",
      "from zora's domain",
      "from ice cavern",
      "south-west corner",
      "from great fairy's fountain",
    },
  },
  {
    "gerudo valley",
    5, (const char*[])
    {
      "from hyrule field",
      "thrown out of gerudo's fortress",
      "on gerudo's fortress loading zone",
      "from gerudo's fortress",
      "from carpenter's tent",
    },
  },
  {
    "lost woods",
    10, (const char*[])
    {
      "from kokiri forest",
      "from sacred forest meadow",
      "mushroom timeout",
      "opposite of entrance from kokiri forest",
      "right side of entrance from kokiri forest",
      "from exit east of sacred forest meadow",
      "from goron city",
      "from zora's river",
      "bridge, from hyrule field",
      "bridge, from kokiri forest",
    },
  },
  {
    "desert colossus",
    9, (const char*[])
    {
      "from haunted wasteland",
      "from spirit temple",
      "from spirit temple, silver gauntlets",
      "from spirit temple, mirror shield",
      "after learning requiem",
      "from requiem warp",
      "after receiving the silver gauntlets",
      "from great fairy's fountain",
      "from spirit temple warp",
    },
  },
  {
    "gerudo's fortress",
    19, (const char*[])
    {
      "from gerudo valley",
      "from theives' hideout, 1st floor left",
      "from theives' hideout, 1st floor middle-left",
      "from theives' hideout, 1st floor middle",
      "from theives' hideout, 2nd floor middle-right",
      "from theives' hideout, 1st floor right",
      "from theives' hideout, 2nd floor middle-left",
      "from theives' hideout, 2nd floor right",
      "from theives' hideout, 3rd floor right",
      "from theives' hideout, 3rd floor middle-right",
      "from theives' hideout, 4th floor middle",
      "from theives' hideout, 2nd floor left",
      "from theives' hideout, above prison",
      "from theives' hideout, 3rd floor left",
      "from gerudo training grounds",
      "from haunted wasteland",
      "after archery game",
      "captured first time",
      "captured 2nd+ time",
    },
  },
  {
    "haunted wasteland",
    3, (const char*[])
    {
      "from gerudo's fortress",
      "from desert colossus",
      "(bad map, crashes)",
    },
  },
  {
    "hyrule castle",
    5, (const char*[])
    {
      "from market",
      "from castle courtyard",
      "from great fairy's fountain",
      "caught in castle courtyard",
      "caught outside hyrule castle",
    },
  },
  {
    "death mountain trail",
    6, (const char*[])
    {
      "from kakariko village",
      "from goron city",
      "from death mountain crater",
      "from dodongo's cavern",
      "from great fairy's fountain",
      "after receiving the goron's ruby",
    },
  },
  {
    "death mountain crater",
    6, (const char*[])
    {
      "from death mountain trail",
      "from goron city",
      "from fire temple",
      "from great fairy's fountain",
      "from bolero warp",
      "from fire temple warp",
    },
  },
  {
    "goron city",
    4, (const char*[])
    {
      "from death mountain trail",
      "from death mountain crater",
      "from goron shop",
      "from lost woods",
    },
  },
  {
    "lon lon ranch",
    11, (const char*[])
    {
      "from hyrule field",
      "after learning epona's song",
      "near south gate",
      "near west gate",
      "from talon's house",
      "from stable",
      "horse riding",
      "after beating ingo",
      "after losing to ingo",
      "near ingo / malon",
      "from back tower",
    },
  },
  {
    "ganon's castle",
    5, (const char*[])
    {
      "from market",
      "from inside ganon's castle",
      "from great fairy's fountain",
      "caught in castle courtyard",
      "(bad map, crashes)"
    },
  },
  {
    "collision test",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "ganondorf test room",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "depth test",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "stalfos middle room",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "stalfos boss room",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "dark link test",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "beta castle courtyard",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "action test",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "item test",
    1, (const char*[])
    {
      "default",
    },
  },
  {
    "lost scene",
    1, (const char*[])
    {
      "default",
    },
  },
};
